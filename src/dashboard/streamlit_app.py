...
# --- PAGE CONFIGURATION ---
st.set_page_config(
    page_title="Vietnam Air Quality AI - Showcase",
    page_icon="🤖",
    layout="wide"
)
...
# --- CITY COORDINATES ---
CITY_COORDS = {
    "Hanoi": [21.0285, 105.8542], "Ha Noi": [21.0285, 105.8542],
    "Ho Chi Minh": [10.8231, 106.6297], "Da Nang": [16.0544, 108.2022],
    "Hai Phong": [20.8449, 106.6881], "Can Tho": [10.0452, 105.7469],
    "Ba Ria Vung Tau": [10.4114, 107.1362], "Bac Giang": [21.2731, 106.1946],
    "Bac Ninh": [21.1861, 106.0763], "Binh Dinh": [13.7820, 109.2192],
    "Binh Duong": [10.9805, 106.6519], "Cao Bang": [22.6667, 106.2500],
    "Gia Lai": [13.9833, 108.0000], "Ha Nam": [20.5453, 105.9122],
    "Ha Nam Province": [20.5453, 105.9122], "Hai Duong": [20.9333, 106.3167],
    "Hung Yen": [20.6464, 106.0511], "Hoa Binh Province": [20.8133, 105.3383],
    "Khanh Hoa": [12.2388, 109.1967], "Lam Dong": [11.9404, 108.4583],
    "Lang Son": [21.8533, 106.7583], "Lang Son Province": [21.8533, 106.7583],
    "Lao Cai": [22.4833, 103.9667], "Long An": [10.5333, 106.4167],
    "Nghe An": [18.6667, 105.6667], "Ninh Binh": [20.2539, 105.9750],
    "Ninh Thuan": [11.5667, 108.9833], "Phu Tho": [21.3000, 105.4000],
    "Quang Binh Province": [17.4833, 106.6000], "Quang Nam": [15.5667, 108.4833],
    "Quang Ngai": [15.1167, 108.8000], "Quang Ninh": [20.9500, 107.0833],
    "Quang Ninh Province": [20.9500, 107.0833], "Quang Tri": [16.8000, 107.1000],
    "Son La": [21.3167, 103.9000], "Tay Ninh": [11.3000, 106.1167],
    "Thai Binh": [20.4500, 106.3333], "Thai Binh Province": [20.4500, 106.3333],
    "Thua Thien Hue": [16.4637, 107.5909], "Tra Vinh": [9.9333, 106.3333],
    "Tuyen Quang Province": [21.8167, 105.2167], "Vinh Long": [10.2500, 105.9667],
...
if df is not None:
    ...
    if model is not None:
        ...
        st.markdown("""
        <div class="explanation-box">
        <b>How to interpret the AI Scorecard:</b><br>
        <ul>
            <li><b>R² Score (Accuracy):</b> A score above 80% means the AI understands the pollution patterns very well.</li>
            <li><b>Mean Absolute Error (MAE):</b> The "Average Mistake". If real AQI is 150 and AI guesses 155, error is 5. Lower is better.</li>
        </ul>
        </div>
        """, unsafe_allow_html=True)

        col1, col2, col3 = st.columns(3)
        col1.metric("🧪 Test Samples", len(y_test), "Data points used")
        ...
        with tab1:
            ...
            st.markdown("If the **Orange Dotted Line** (AI) follows the **Blue Line** (Reality) closely, the model is working.")
            fig_ts = go.Figure()
            fig_ts.add_trace(go.Scatter(x=dates_test, y=y_test, mode='lines', name='Reality', line=dict(color='#1f77b4', width=2)))
        ...
        with tab2:
            ...
            st.markdown("Dots on the **Grey Diagonal Line** are perfect predictions. Dots far away are misses.")
        ...
        with tab3:
            ...
            st.markdown("Ideally, errors should be centered at 0 (Bell Curve), meaning mistakes are random, not systematic.")
            residuals = y_test - y_pred
            fig_hist = px.histogram(residuals, nbins=30, labels={'value': 'Prediction Error'}, color_discrete_sequence=['#ef553b'])
            fig_hist.update_layout(showlegend=False)
            st.plotly_chart(fig_hist, use_container_width=True)

        with tab4:
            st.subheader("What drives the AI's decisions?")
            st.markdown("Longer bars indicate which pollutants (PM2.5, CO, etc.) have the most impact on Air Quality.")
